...
export ACARTRIDGE_HASURA_DATABASE_URL
ACARTRIDGE_HASURA_DATABASE_URL="$(node -e "
const u = process.env.ENVIO_PG_USER || 'postgres';
const p = process.env.ENVIO_PG_PASSWORD || 'testing';
const db = process.env.ACARTRIDGE_PG_DATABASE || 'envio-acartridge';
console.log(
  'postgres://' +
...
exec docker compose -f docker/aarcade-acartridge-base/docker-compose.yaml --env-file .env "$@"
