...
export BAZAAR_HASURA_DATABASE_URL
BAZAAR_HASURA_DATABASE_URL="$(node -e "
const u = process.env.ENVIO_PG_USER || 'postgres';
const p = process.env.ENVIO_PG_PASSWORD || 'testing';
const db = process.env.BAZAAR_PG_DATABASE || 'envio-bazaar';
console.log(
  'postgres://' +
...
exec docker compose -f docker/aarcade-bazaar-base/docker-compose.yaml --env-file .env "$@"
